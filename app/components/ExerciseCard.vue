...
<template>
    <div
        :class="['bg-card border rounded-2xl p-4 space-y-3 transition-colors', reorderMode ? 'border-primary-500/30 select-none' : 'border-border']">
        <!-- Header -->
        <div class="flex justify-between items-center gap-2">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <!-- Reorder mode: drag handle  arrow buttons -->
                <template v-if="reorderMode && !isLocked">
                    <button @click.stop="emit('move-up')" :disabled="!canMoveUp"
                        :class="['rounded-lg p-1 transition-colors', canMoveUp ? 'text-primary-400 hover:bg-surface' : 'text-text-muted/30 cursor-not-allowed']">
                        <IconArrowUp class="size-4" />
                    </button>
                    <button @click.stop="emit('move-down')" :disabled="!canMoveDown"
                        :class="['rounded-lg p-1 transition-colors', canMoveDown ? 'text-primary-400 hover:bg-surface' : 'text-text-muted/30 cursor-not-allowed']">
                        <IconArrowDown class="size-4" />
                    </button>
                    <div
                        class="drag-handle cursor-grab active:cursor-grabbing text-text-muted hover:text-text touch-none p-1">
                ...
                <template v-else-if="!isLocked">
                    ...
                    <template v-if="confirmingDelete">
                        <button @click="emit('delete'); confirmingDelete = false"
        ...
        <div v-if="exercise.type === 'strength'" class="flex flex-wrap gap-2">
            <div v-for="(set, i) in exercise.sets" :key="i"
                ...
                <!-- reps or reps+weight -->
                <template v-else>
                    <span class="font-medium">{{ set.reps ?? '—' }}<span class="text-text-muted font-normal">
                            reps</span></span>
                    <template v-if="exercise.mode !== 'reps' && set.weight">
                        <span class="text-text-muted">•</span>
                        <span class="font-medium text-primary-400">{{ set.weight }}<span
        ...
        <div v-if="exercise.type === 'cardio'" class="flex flex-wrap gap-2">
            ...
            <div v-if="exercise.metric && exercise.metric !== 'none' && exercise.metricValue != null"
                ...
                <span class="font-medium">{{ exercise.metricValue }}<span class="text-text-muted font-normal"> {{
                    exercise.metric === 'speed' ? 'km/h' : '' }}</span></span>
        ...
        <div v-if="exercise.muscleGroups?.length" class="flex flex-wrap gap-1">
            <span v-for="mg in exercise.muscleGroups" :key="mg"
                class="text-xs px-2 py-0.5 rounded-full bg-surface text-text-muted border border-border/60">{{ mg
                }}</span>
        </div>
    </div>
</template>
